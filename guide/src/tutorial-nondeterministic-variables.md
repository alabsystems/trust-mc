...
trust-mc is able to reason about programs and their execution paths by allowing users to create nondeterministic (also called symbolic) values using `kani::any()`.
trust-mc is a "bit-precise" model checker, which means that trust-mc considers all the possible bit-value combinations _that would be valid_ if assigned to a variable's memory contents.
...
Out of the box, trust-mc includes `kani::any()` implementations for most primitive and some `std` types.
...
While `kani::any()` is the only method trust-mc provides to inject non-determinism into a proof harness, trust-mc only ships with implementations for a few `std` types where we can guarantee safety.
...
This will enable you and downstream crates to use trust-mc via [Bolero](https://camshaft.github.io/bolero/).
...
trust-mc includes a derive macro that allows you to automatically derive `kani::Arbitrary` for structures and enumerations as long as all its fields also implement the `kani::Arbitrary` trait.
One downside of this approach today is that the `kani` crate ships with trust-mc, but it's not yet available on [crates.io](https://crates.io).
...
The benefits of doing so were described in [this Kani blog post](https://model-checking.github.io/kani-verifier-blog/2022/10/27/using-kani-with-the-bolero-property-testing-framework.html).
...
> But trust-mc just see 3 meaningful possibilities, each of which is not treated any differently from each other.
...
trust-mc does not (yet!) scale well to nondeterministic-size data structures involving heap allocations.
...
1. We saw how `kani::any()` will return "safe" values for each of the types trust-mc implements it for.
