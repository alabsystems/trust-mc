...
If we try removing this "unwind" annotation and re-running trust-mc, the result is worse: non-termination.
trust-mc simply doesn't produce a result.

The problem we're struggling with is the technique trust-mc uses to verify code.
...
To verify programs like this with trust-mc as it exists today, we need to do two things:
...
2. Tell trust-mc about this limit if (or when) it's not able to figure it out on its own.
...
The "unwinding assertion" failure that trust-mc reports is because this bound is not high enough.
The code tries to execute more than 1 loop iteration.
(And, because the unwinding isn't high enough, many of the other properties trust-mc is verifying become "undetermined": we don't really know if they're true or false, because we can't get far enough.)
...
But that's not large enough for trust-mc, and we still see the "unwinding assertion" failure.
...
trust-mc needs the unwinding bound to be "one more than" the number of loop iterations.
...
The best approach to supplying trust-mc with unwind bounds is using the annotation `kani::unwind`, as we show above.
...
trust-mc frequently needs to do "bounded proof", which contrasts with unbounded or full verification.

We've written a proof harness that shows `initialize_prefix` has no errors on input slices of size 10, but no higher.
The particular size we choose is usually determined by balancing the level of assurance we want, versus runtime of trust-mc.
...
One consequence of this, however, is that trust-mc often scales poorly to "big string problems" like parsing.
...
1. We saw trust-mc fail to terminate.
2. We saw how `#[kani::unwind(1)]` can help force trust-mc to terminate (with a verification failure).
