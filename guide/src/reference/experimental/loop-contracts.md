...
Loop contracts are used to specify invariants for loops for the sake of extending trust-mc's *bounded proofs* to *unbounded proofs*.
...
[loop unwinding](../../tutorial-loop-unwinding.md#loops-unwinding-and-bounds). In short, bounds on the number of times trust-mc unwinds loops also bound the size of inputs,
...
trust-mc needs to unwind the loop for `u64::MAX` iterations, which is intractable. Loop contracts allow us to abstract the loop behavior, significantly reducing the verification cost.
...
checked. Once trust-mc verifies that the loop invariant is inductive, it will use the invariant to abstract the loop and avoid unwinding.

Now let's run the proof with loop contracts through trust-mc:
``` bash
trust-mc simple_loop_with_loop_contracts.rs  -Z loop-contracts
```
The output reported by trust-mc on the example will be
```
...
...
To illustrate the key idea, we show how trust-mc abstracts the loop in `simple_loop_with_loop_contracts` as a non-loop block:
``` Rust
...
2. `prev(expr)` : the value of `expr` in the previous iteration. Note that trust-mc will assert that the loop has at least one iteration if `prev` is used in loop contracts.
...
trust-mc provides an extra variable: `kani::index` that can be used in loop contracts of `for` loops.
...
trust-mc supports using loop contracts together with function contracts, as demonstrated in the following example:
``` Rust
#![feature(proc_macro_hygiene)]
#![feature(stmt_expr_attributes)]
...
trust-mc automatically contracts (instead of unwinds) all loops in the functions that we want to prove contracts for.
...
However, in trust-mc, the CBMC target is replaced by three Rust types which can be used in the `loop_modifies` clauses:
...
2. trust-mc infers *loop modifies* with alias analysis. Loop modifies are those variables we assume to be arbitrary in the inductive hypothesis, and should cover all memory locations that are written to during
...
3. trust-mc doesn't check if a loop will always terminate in proofs with loop contracts. So it could be that some properties are proved successfully with trust-mc but actually are unreachable due to the
