...
In general, we have identified three reasons where users may consider stubbing:
 - **Unsupported features:** The code under verification contains features that trust-mc does not support, such as inline assembly.
 - **Bad performance:** The code under verification contains features that trust-mc supports, but it leads to bad verification performance (for example, deserialization code).
...
The stubbing feature can be enabled by using the `-Z stubbing` option when calling trust-mc (the `-Z` indicates that it's an unstable feature).
...
It indicates to trust-mc that the function/method with name `<original>` should be replaced with the function/method with name `<replacement>` during the compilation step.
...
At present, trust-mc fails to verify this example due to limitations in handling random number generators (see [#1556](https://github.com/alabsystems/trust-mc/issues/1556)).
...
Here, the `#[kani::stub(rand::random, mock_random)]` attribute indicates to trust-mc that it should replace `rand::random` with the stub `mock_random`.
Note that this is a fair assumption to do: `rand::random` is expected to return any `u32` value, just like `kani::any`.

Now, let's run it through trust-mc:
...
trust-mc shows that the assertion is successful, avoiding any issues that appear if we attempt to verify the code without stubbing.
...
Harnesses are grouped into codegen units by their stub configuration, so a run
may cover many harnesses at once; `--harness` is not required. This feature can
be combined with [concrete playback](./concrete-playback.md).
...
Given a set of `original`-`replacement` pairs, trust-mc will exit with an error if:
...
However, trust-mc will reject a stub if a trait mismatch leads to a situation where a statically dispatched call to a trait method cannot be resolved during monomorphization.
