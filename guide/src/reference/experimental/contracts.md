...
It would be very expensive for trust-mc to unroll each of these iterations and then perform symbolic execution.

Instead, we can write *contracts* with guarantees about `gcd`'s behavior.
Once trust-mc verifies that `gcd`'s contracts are correct, it can replace each invocation of `gcd` with its contracts, which reduces verification time for `gcd`'s callers.
...
The `requires` contract tells trust-mc to restrict the range of nondeterministic inputs to nonzero ones so that we don't run into this error.
...
and verify it by running `trust-mc -Z function-contracts`.

Once trust-mc verifies the contracts, we can use trust-mc's [stubbing feature](stubbing.md) to replace all invocations to `gcd` with its contracts, for instance:

```rust
// Assume foo() invokes gcd().
// By using stub_verified, we tell trust-mc to replace
...
We highly recommend reading the [Kani blog post about contracts](https://model-checking.github.io/kani-verifier-blog/2024/01/29/function-contracts.html) (from which this `gcd` example is taken; trust-mc inherits Kani's contracts feature). We also recommend looking at the `contracts` module in our [documentation](../../crates/index.md).
