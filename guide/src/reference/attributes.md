...
In trust-mc, attributes are used to mark functions as harnesses and control their execution.
This section explains the attributes available in trust-mc and how they affect the verification process.

At present, the available trust-mc attributes are the following:
...
If we run trust-mc on this example:
...
This will be followed by multiple messages that come from AY (the verification engine used by trust-mc) and the [verification results](../verification-results.md).

Using any other trust-mc attribute without `#[kani::proof]` will result in compilation errors.
...
Running trust-mc on it will produce the result `VERIFICATION:- SUCCESSFUL (encountered one or more panics as expected)`

## `#[kani::unwind(<number>)]`

**The `#[kani::unwind(<number>)]` attribute specifies that all loops must be unwound up to `<number>` times.**

By default, trust-mc attempts to unwind all loops automatically.
...
After the unwinding stage, trust-mc will attempt to verify the harness.
...
Running this example on trust-mc will produce a successful verification result.
In this case, trust-mc automatically finds the required unwinding value (i.e., the number of times it needs to unwind all loops).
...
> **NOTE**: trust-mc verifies with AY only (`--backend=auto|ay`), so this attribute has no effect in trust-mc.
> It is still parsed for source compatibility with upstream Kani.
> Use `--smt-solver` (or alias `--ay-solver`) to choose the AY solver.

**Compatibility attribute inherited from upstream Kani; ignored by trust-mc's active backend.**

If you share code with upstream Kani, you may still see this attribute in examples.
In trust-mc, solver selection is controlled from the CLI, not from this attribute.

Upstream Kani historically accepted these `<solver>` values:
...
The following attribute is parsed, but trust-mc still uses CLI-based AY solver selection:

```rust
#[kani::proof]
#[kani::solver(cadical)]  // Compatibility attribute; ignored by trust-mc backend.
fn check() {
    let mut a = [2, 3, 1];
    a.sort();
    assert_eq!(a[0], 1);
...
Changing this attribute does not change trust-mc verification behavior.

Use `--ay-solver`/`--smt-solver` if you want to compare AY-based solver modes.
