# Failures that trust-mc can spot

In the [last section](./tutorial-first-steps.md), we saw trust-mc spot two major kinds of failures: assertions and panics.
If the proof harness allows some program execution that results in a panic, then trust-mc will report that as a failure.
In addition, we saw (very briefly) a couple of other kinds of failures: null pointer dereferences and overflows.
In this section, we're going to expand on these additional checks, to give you an idea of what other problems trust-mc will find.
...
But if we write a trust-mc proof harness:
...
> trust-mc is inserting a lot more checks than appear as asserts in our code, so the output can be large.
...
Notice that, for trust-mc, this has gone from a simple bounds-checking problem to a pointer-checking problem.
trust-mc will check operations on pointers to ensure they're not potentially invalid memory accesses.
...
1. Exercise: Switch back to the normal/safe indexing operation and re-try trust-mc.
How does trust-mc's output change, compared to the unsafe operation?
(Try predicting the answer, then seeing if you got it right.)
2. Exercise: Try trust-mc's experimental [concrete playback](reference/experimental/concrete-playback.md) feature on this example.
3. Exercise: Fix the error, run trust-mc, and see a successful verification.
4. Exercise: Try switching back to the unsafe code (now with the error fixed) and re-run trust-mc. Does it still verify successfully?

<details>
<summary>Click to see explanation for exercise 1</summary>

Having switched back to the safe indexing operation, trust-mc reports a bounds check failure:
...
trust-mc will spot this not as a bound error, but as a mathematical error: on an empty list the modulus operator (`%`) will cause a division by zero.

1. Exercise: Try to run trust-mc on this version of `get_wrapped`, to see what this kind of failure looks like.
...
trust-mc will find these failures as well.
...
trust-mc will then report no issues.
...
## Failures that trust-mc cannot spot

Check out [Limitations](./limitations.md) for information on the checks that trust-mc does not perform.
Notably, trust-mc is not prioritizing all Rust-specific notions of undefined behavior.

## Summary

In this section:

1. We saw trust-mc spot out-of-bounds accesses.
2. We saw trust-mc spot actually-unsafe dereferencing of a raw pointer to invalid memory.
3. We saw trust-mc spot a division by zero error and an overflowing addition.
