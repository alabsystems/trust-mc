...
This section collects frequently asked questions about trust-mc.
Please consider [opening an issue](https://github.com/alabsystems/trust-mc/issues/new/choose) if you have a question that would like to see here.

## Questions

<details>
<summary>trust-mc doesn't fail after <code>kani::assume(false)</code>. Why?</summary>
<br>

`kani::assume(false)` (or `kani::assume(cond)` where `cond` is a condition that results in `false` in the context of the program), won't cause errors in trust-mc.
...
That's the expected behavior for `kani::assume(false)` in trust-mc.
...
More details about this option, which also useful in other cases, can be found [here](tutorial-nondeterministic-variables.md#custom-nondeterministic-types).

If the type comes from `std` (Rust's standard library), you can [open a request](https://github.com/alabsystems/trust-mc/issues/new?assignees=&labels=%5BC%5D+Feature+%2F+Enhancement&template=feature_request.md&title=) for adding `Arbitrary` implementations to the trust-mc library.
