...
Running trust-mc on a harness produces an output that includes a set of checks as
follows:
...
The result (or `Status`) of a check in trust-mc can be one of the following:
...
```
The output from trust-mc indicates that the assertion holds:
```
...
```
The assertion doesn't hold as trust-mc's output indicates:
```
...
trust-mc currently checks reachability for the following assertion types:
...
The output from trust-mc indicates that the assertion is unreachable:
```
...
4. `UNDETERMINED`: This indicates that trust-mc was not able to conclude whether the
property holds or not. This can occur when the Rust program contains a construct
that is not currently supported by trust-mc. See
[Rust feature support](./rust-feature-support.md) for trust-mc's current support of the
...
```
The output from trust-mc indicates that the assertion is undetermined due to the
missing support for inline assembly in trust-mc:
```
...
trust-mc provides a `kani::cover` macro that can be used for checking whether a condition may occur at a certain point in the code. (See [upstream Kani documentation](https://model-checking.github.io/kani/crates/doc/kani/macro.cover.html) for API details.)

The result of a cover property can be one of the following:

1. `SATISFIED`: This indicates that trust-mc found an execution that triggers the specified condition.
...
2. `UNSATISFIABLE`: This indicates that trust-mc _proved_ that the specified condition is impossible.
...
trust-mc reports a summary at the end of the verification report, which includes the overall results of all checks, the overall results of cover properties (if the package includes cover properties), and the overall verification result, e.g.:
```
SUMMARY:
