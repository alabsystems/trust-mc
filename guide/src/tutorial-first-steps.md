...
trust-mc is unlike the testing tools you may already be familiar with.
...
Let's write a trust-mc [_proof harness_](reference/attributes.md#kaniproof) for `estimate_size`.
...
trust-mc has immediately found a failure.
Notably, we haven't had to write explicit assertions in our proof harness: by default, trust-mc will find a host of erroneous conditions which include a reachable call to `panic` or a failing `assert`.
If trust-mc had run successfully on this harness, this amounts to a mathematical proof that there is no input that could cause a panic in `estimate_size`.

> By default, trust-mc only reports failures, not how the failure happened.
> In this example, it would be nice to get a concrete example of a value of `x` that triggers the failure.
> trust-mc offers an (experimental) [concrete playback](reference/experimental/concrete-playback.md) feature that serves this purpose.
> As an exercise, try applying concrete playback to this example and see what trust-mc outputs.


### Exercise: Try other failures

We put an explicit panic in this function, but it's not the only kind of failure trust-mc will find.
...
But trust-mc still catches the issue:
...
**Exercise: Can you find an example where the Rust compiler will not complain, and trust-mc will?**
...
Overflow (in addition, multiplication or, in this case, [bit-shifting by too much](https://github.com/rust-lang/rust/issues/10183)) is also caught by trust-mc:
...
1. We saw trust-mc find panics, assertion failures, and even some other failures like unsafe dereferencing of null pointers.
2. We saw trust-mc find failures that testing could not easily find.
