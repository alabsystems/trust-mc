...
trust-mc is developed on the top of the Rust compiler, which is not distributed on [crates.io](https://crates.io/) and depends on
...
To debug trust-mc in VS code, first install the [CodeLLDB extension](https://marketplace.visualstudio.com/items?itemName=vadimcn.vscode-lldb).
...
Note that pretty printing for the Rust nightly toolchain (which trust-mc uses) is not very good as of June 2022.
...
Open the `Cargo.toml` of your crate (e.g.: `trust-mc-compiler`), and do the following:
...
For LSP to be able to find `rustc_private` files used by trust-mc, you
...
`rustc` is the same version and architecture as what trust-mc uses. If
...
We will assume that you already have a trust-mc setup and that the variable `TRUST_MC_WORKSPACE` contains the path to your trust-mc workspace.
...
```bash
cd ${TRUST_MC_WORKSPACE} # Go to your trust-mc workspace.
...
Finally, override the current toolchain in your trust-mc workspace and rebuild trust-mc:
```bash
cd ${TRUST_MC_WORKSPACE}
rustup override set custom-toolchain
cargo clean
cargo build-dev
```
# Rust compiler utilities to debug `trust-mc-compiler`
...
For logs that are related to `trust-mc-compiler` code, use the `TRUST_MC_LOG` variable (the deprecated `KANI_LOG` is also accepted as a fallback).
...
This flag can be passed to `trust-mc` or `targo trust-mc` by setting the `RUSTFLAGS` environment variable.
...
```
RUSTFLAGS=--emit=mir trust-mc test.rs
```
