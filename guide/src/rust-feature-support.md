...
The table below tries to summarize the current support in trust-mc for
 ...
 * **Yes**: The feature is expected to work for covered regression cases. This is not a replacement-quality guarantee for every use.
 * **Partial**: The feature is at least partially supported. We are aware of some issue with it.
...
would greatly appreciate that you [filed a bug report](https://github.com/alabsystems/trust-mc/issues/new?assignees=&labels=bug&template=bug_report.md).

This table tracks language-feature support, not full Kani replacement status. Replacement claims are
governed by the proof inventory and audit process in `replacement-proof.md` at the repository root.
...
6.10 | Static items | Partial | `static extern` items and uninitialized statics have known caveats |
6.11 | Traits | Yes | |
6.12 | Implementations | Yes | |
6.13 | External blocks | Partial | `static extern` items have known caveats |
...
8.2.6 | Array and index expressions | Partial | Non-`Copy` array iterators have known caveats |
...
8.2.12 | Closure expressions | Partial | See [Notes - Advanced features](#advanced-features) |
...
8.2.18 | Await expressions | Partial | `async`/`await` support is incomplete |
9 | Patterns | Partial | [#1548](https://github.com/alabsystems/trust-mc/issues/1548) |
10.1.1 | Boolean type | Yes | |
10.1.2 | Numeric types | Yes | See [Notes - Floats](#floating-point-operations) |
10.1.3 | Textual types | Partial | String and boxed string-slice operations have known caveats |
10.1.4 | Never type | Yes | |
10.1.5 | Tuple types | Yes | |
10.1.6 | Array types | Partial | Non-`Copy` array iterators have known caveats |
...
| | `Box<T>` | Partial | `Box<str>` operations have known caveats |
| | `Rc<T>` | Yes | |
| | `Arc<T>` | Yes | |
| | `Pin<T>` | Yes | |
...
trust-mc aims to be an industrial verification tool. Most industrial crates may
include unsupported features in parts of their code that do not need to be
verified. In general, this should not prevent users using trust-mc to verify their code.

Because of that, the general rule is that trust-mc generates an `assert(false)`
...
In a few cases, trust-mc aborts execution if the analysis could be affected in
some way because of an unsupported feature (e.g., global ASM).

### Assembly

trust-mc does not support assembly code for now. We may add it in the future but at
present there are no plans to do so.

Check out the tracking issues for [inline assembly (`asm!`
macro)](https://github.com/alabsystems/trust-mc/issues/1545) and [global assembly
(`global_asm!` macro)](https://github.com/alabsystems/trust-mc/issues/1546) to know
more about the current status.

### Concurrency

Concurrent features are currently out of scope for trust-mc. In general, the
...
Because of this, trust-mc emits a warning whenever it encounters concurrent code and
compiles as if it was sequential code.

### Standard library functions

trust-mc [overrides](./overrides.md) a few common functions
...
[#1549](https://github.com/alabsystems/trust-mc/issues/1549).
If you become aware of other issues concerning
these features, please [file a bug
report](https://github.com/alabsystems/trust-mc/issues/new?assignees=&labels=bug&template=bug_report.md).
...
trust-mc has partial, experimental support for detecting access to uninitialized memory with the `-Z uninit-checks` option.
See [upstream Kani #3300](https://github.com/model-checking/kani/issues/3300) for CBMC-specific soundness details.
...
on the current support in trust-mc for Rust compiler intrinsics.

### Floating point operations

> **Backend caveat (CHC vs BMC).** Floats are fully modeled with SMT-FP only on
> the **bounded (BMC)** path. On the **CHC** path (`--ay-chc`), a float local
> currently records the demoting `fp_bitvector_encoding` construct and symbolic
> float arithmetic is not carried into the PDR engine, so float-bearing harnesses
> there are demoted (cannot be *proved*). Until CHC carries SMT-FP theory, route
> float harnesses through BMC if you need a proof. The "Yes" for Numeric types in
> the table above therefore means "proved under BMC; demoted under CHC."

trust-mc supports floating point numbers, but some supported operations on floats are "over-approximated."
...
For instance, ([#1551](https://github.com/alabsystems/trust-mc/issues/1551)) the `sin`/`cos` functions basically return a nondeterministic value between -1 and 1.
In other words, they largely ignore their input and give very conservative answers.
This range certainly includes the "real" value, so proof soundness is still preserved, but it means trust-mc could raise spurious errors that cannot actually happen.
This makes trust-mc unsuitable for verifying some kinds of properties (e.g. precision) about numerical algorithms.
