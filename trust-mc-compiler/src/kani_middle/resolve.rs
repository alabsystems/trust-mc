...
/// # Contracts
///
/// REQUIRES: tcx is valid for the current compilation session
/// REQUIRES: current_module is a valid LocalDefId in the current crate
/// REQUIRES: path is a syntactically valid TypePath
/// ENSURES: Ok(FnResolution::Fn(def)) implies def is a function or method
/// ENSURES: Ok(FnResolution::FnImpl { def, ty }) implies def implements a trait method for ty
/// ENSURES: Err(e) describes why resolution failed (path not found, wrong type, etc.)
pub(crate) fn resolve_fn_path<'tcx>(
    tcx: TyCtxt<'tcx>,
    ...
    match &path.qself {
        ...
        Some(QSelf { ty: syn_ty, position, .. }) if *position > 0 => {
            ...
            // Extract the trait's own generic arguments, e.g. `u32` in
            // `<MyType as Convert<u32>>::convert`, so we can disambiguate between
            // multiple impls of the same trait for the same Self type (#1997).
            // The trait segment is the one immediately before the qself split.
            let trait_generic_args = path
                .path
                .segments
                .iter()
                .nth(position.saturating_sub(1))
                .map(|seg| resolve_trait_generic_args(tcx, current_module, &seg.arguments))
                .transpose()?
                .unwrap_or_default();
            resolve_in_trait_impl(tcx, ty, trait_fn_id, trait_generic_args)
        }
        ...
        Some(QSelf { ty: syn_ty, .. }) if type_resolution::is_type_primitive(syn_ty) => {
            ...
            if resolved.segments.is_empty() {
                Ok(FnResolution::Fn(
                    stable_fn_def(tcx, resolved.base)
                        .expect("resolved base should convert to stable FnDef"),
                ))
            } else {
        ...
        Some(QSelf { ty: syn_ty, .. }) => {
            ...
            Ok(FnResolution::Fn(
                stable_fn_def(tcx, def_id).expect("def_id should convert to stable FnDef"),
            ))
        }
        // Simple path
        None => {
...
/// REQUIRES: tcx is valid for the current compilation session
/// REQUIRES: current_module is a valid LocalDefId in the current crate
/// REQUIRES: path_str is syntactically parseable as a Rust path (validated via syn::parse_str)
/// ENSURES: Ok(def_id) implies def_id is a function or method definition
/// ENSURES: Err(e) describes parse failure or resolution failure
pub(crate) fn resolve_fn<'tcx>(
    tcx: TyCtxt<'tcx>,
    current_module: LocalDefId,
    path_str: &str,
...
fn resolve_path<'tcx>(
    ...
    path.segments
        .into_iter()
        .try_fold((path.base, path.base_path_args), |(base, base_path_args), segment| {
...
pub(crate) enum ResolveError<'tcx> {
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            ResolveError::AmbiguousGlob { tcx, base, name, candidates } => {
                ...
                let paths: Vec<_> =
                    candidates.iter().map(|def_id| tcx.def_path_str(*def_id)).collect();
                write!(
                    f,
            ...
            ResolveError::UnsupportedPath { kind } => {
                write!(f, "trust_mc currently cannot resolve {kind}")
            }
        }
    }
}
...
fn resolve_prefix<'tcx>(
    ...
    match (path.leading_colon, segments.next()) {
        ...
        (None, Some(segment)) if type_resolution::is_primitive(&segment) => {
            let syn_ty =
                syn::parse2(segment.to_token_stream()).expect("segment should parse as syn type");
            let ty = type_resolution::resolve_ty(tcx, current_module, &syn_ty)?;
        ...
        _ => unreachable!("Empty path: `{path:?}`"), // non-enum: tuple (None, None) case
    }
}
    ...
    let matches: Vec<FnResolution> = rustc_public::all_trait_decls()
        ...
        .filter_map(|trait_def| {
            resolve_in_trait_def_stable(tcx, trait_def, name).ok().and_then(|item| {
                // Unqualified name (e.g. bare `convert`): no trait generic args
                // are written, so rely on Self-type selection only.
...
/// Resolve the angle-bracketed generic *type* arguments written on a trait
/// segment (e.g. `u32` in `Convert<u32>`) into rustc_public `GenericArgKind`s.
///
/// Lifetime arguments are dropped (they are erased for instance selection).
/// If any argument is not a plain type (e.g. a const generic) we conservatively
/// return an empty list so the caller falls back to Self-only selection rather
/// than building a malformed argument vector.
fn resolve_trait_generic_args<'tcx>(
    tcx: TyCtxt<'tcx>,
    current_module: LocalDefId,
    arguments: &PathArguments,
) -> Result<Vec<GenericArgKind>, ResolveError<'tcx>> {
    ...
    for arg in &angle.args {
        match arg {
            syn::GenericArgument::Type(syn_ty) => {
                let ty = type_resolution::resolve_ty(tcx, current_module, syn_ty)?;
                resolved.push(GenericArgKind::Type(ty));
            }
...
fn resolve_in_trait_impl(
    ...
    ty: Ty,
    trait_fn_id: DefId,
    trait_generic_args: Vec<GenericArgKind>,
) -> Result<FnResolution, ResolveError<'_>> {
    debug!(?ty, ?trait_generic_args, "resolve_in_trait_impl");
    // Given the `FnDef` of the *definition* of the trait method, see if there exists an Instance
    // that implements that method for `ty`.
    let trait_fn_fn_def =
        stable_fn_def(tcx, trait_fn_id).expect("trait function should convert to stable FnDef");
    // A trait method's generics are `[Self, <trait params...>, <method params...>]`.
    // The Self type plus any trait params (e.g. `T` in `Convert<T>`) are what
    // select the impl; method-level params are inferred by `Instance::resolve`.
    let mut generic_args = vec![GenericArgKind::Type(ty)];
    generic_args.extend(trait_generic_args);
    let desired_generic_args = GenericArgs(generic_args);
    ...
    exists.map_or(Err(ResolveError::MissingTraitImpl { tcx, trait_fn_id, ty }), |inst| {
        Ok(FnResolution::FnImpl {
            def: inst.ty().kind().fn_def().expect("instance should have fn_def").0,
            ty,
        })
    })
}
        ...
        if item.kind.ident().is_some_and(|ident| ident.as_str() == name) {
            match item.kind {
                ItemKind::Use(use_path, UseKind::Single(_)) => {
                    use_path.res.present_items().find_map(|res| res.opt_def_id())
                }
                ItemKind::ExternCrate(orig_name, _) => resolve_external(
                    tcx,
                    orig_name.as_ref().map_or(name, rustc_span::Symbol::as_str),
                ),
                _ => Some(item.owner_id.def_id.to_def_id()), // external enum: ItemKind
            }
        } else {
            // Check ForeignMod blocks for extern function declarations (#983)
            if let ItemKind::ForeignMod { items, .. } = item.kind {
                for foreign_item_id in items {
                    ...
                    if foreign_item.ident.as_str() == name {
                        return Some(foreign_item.owner_id.def_id.to_def_id());
                    }
                }
            }
...
fn resolve_in_glob_uses<'tcx>(
    ...
    match matches.len() {
        ...
        1 => Ok(matches.pop().expect("matches vec has exactly one element")),
    ...
    I: Iterator<Item = &'a PathSegment>,
{
    let def_id = match ty.kind() {
        TyKind::RigidTy(rigid_ty) => match rigid_ty {
            ...
            _ => unreachable!("Unexpected type {ty}"), // external enum: RigidTy
        },
        ...
        TyKind::Param(_) | TyKind::Bound(_, _) => {
            ...
            unreachable!(
                "type parameters and bound variables should be resolved before name resolution"
            )
        }
    };
    let Some(name) = segments.next() else {
        unreachable!(
            "path segments must have at least one remaining segment for the associated item name"
        )
    };
...
fn generic_args_to_string<T: ToTokens>(args: &T) -> String {
    let mut s = args.to_token_stream().to_string();
...
/// Resolves a function in a trait definition.
/// Deprecated: prefer `resolve_in_trait_def_stable` which uses stable MIR types.
/// Removal blocked on callsite at line 166 which passes `DefId` (internal type).
/// Upstream: <https://github.com/model-checking/kani/issues/4252>
    ...
    if let Some(next) = segments.next() {
        ...
        let simple_ty =
            fast_reject::simplify_type(tcx, internal_ty, TreatParams::InstantiateWithInfer)
                .expect("primitive type should simplify");
        ...
        let item = impls
            ...
            .ok_or(ResolveError::MissingPrimitiveItem { base: ty, unresolved: name })?;
        Ok(Path {
            base: item,
            base_path_args: PathArguments::None,
            segments: segments.cloned().collect(),
...
fn is_item_name(tcx: TyCtxt, item: DefId, name: &str) -> bool {
    ...
    let last = item_path.split("::").last().expect("def_path_str should have at least one segment");
    last == name
}
...
fn last_two_items_of_path_match(item_path: &str, generic_args: &str, name: &str) -> bool {
    ...
    // Compare without allocating format strings: chain the segments as char iterators.
    let actual_chars = "::"
        .chars()
        .chain(parts[parts.len() - 2].chars())
        .chain("::".chars())
        .chain(parts[parts.len() - 1].chars())
        .filter(|c| !c.is_whitespace());
    let expected_chars = generic_args.chars().chain("::".chars()).chain(name.chars());

    expected_chars.eq(actual_chars)
}
...
mod tests {
    ...
    mod edge_case_last_two_items_of_path_match {
        use crate::kani_middle::resolve::last_two_items_of_path_match;

        #[test]
        fn single_part_path_returns_false() {
            // No `::` separator → parts.len() == 1 → false
            assert!(!last_two_items_of_path_match("just_a_name", "", "just_a_name"));
        }

        #[test]
        fn two_part_path_no_generics_returns_false() {
            // Without generic_args the type name doesn't form a matching suffix.
            // The function is designed for paths with generics — empty generic_args
            // cannot match because actual_last_two includes the type name.
            assert!(!last_two_items_of_path_match("Foo::bar", "", "bar"));
        }

        #[test]
        fn two_part_path_wrong_name() {
            assert!(!last_two_items_of_path_match("Foo::<u32>::bar", "::<u32>", "baz"));
        }

        #[test]
        fn two_part_path_with_generics() {
            // Exactly two path parts with generics
            assert!(last_two_items_of_path_match("Foo::<u32>::bar", "::<u32>", "bar"));
        }

        #[test]
        fn nested_angle_brackets() {
            // Deeply nested generics: Vec<HashMap<K, V>>
            let generic_args = "::<HashMap<K,V>>";
            let name = "push";
            let item_path = "std::vec::Vec::<HashMap<K, V>>::push";
            assert!(last_two_items_of_path_match(item_path, generic_args, name));
        }

        #[test]
        fn wrong_function_name_with_generics() {
            let generic_args = "::<u32>";
            let item_path = "core::num::NonZero::<u32>::unchecked_add";
            assert!(!last_two_items_of_path_match(item_path, generic_args, "checked_add"));
        }

        #[test]
        fn empty_path_returns_false() {
            assert!(!last_two_items_of_path_match("", "", "anything"));
        }

        #[test]
        fn colons_inside_generics_not_split() {
            // `::` inside angle brackets should not be treated as path separator
            let generic_args = "::<std::collections::HashMap<K,V>>";
            let name = "new";
            let item_path = "alloc::boxed::Box::<std::collections::HashMap<K, V>>::new";
            assert!(last_two_items_of_path_match(item_path, generic_args, name));
        }
    }
}
