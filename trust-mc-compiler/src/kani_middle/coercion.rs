...
#[allow(clippy::panic)] // Internal validation - source/dest must be pointers
pub(crate) fn extract_unsize_casting<'tcx>(
    ...
    .last()
    .expect("coerce unsized iterator should have at least one element");
...
impl Iterator for CoerceUnsizedIterator<'_> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        // the conversion.
        let src_ty = self.src_ty.take().expect("src_ty should be present in iterator");
        let dst_ty = self.dst_ty.take().expect("dst_ty should be present in iterator");
        let field = if let (
        ...
        ) = (src_ty.kind(), dst_ty.kind())
        {
            ...
            let src_fields = &src_def
                .variants_iter()
                .next()
                .expect("ADT should have at least one variant")
                .fields();
            let dst_fields = &dst_def
                .variants_iter()
                .next()
                .expect("ADT should have at least one variant")
                .fields();
            ...
            // external enum: (TyKind, TyKind) — base pointer case
...
fn custom_coerce_unsize_info<'tcx>(
    ...
    match tcx.codegen_select_candidate(PseudoCanonicalInput {
        ...
        Ok(ImplSource::UserDefined(ImplSourceUserDefinedData { impl_def_id, .. })) => tcx
            ...
            .expect("coerce_unsized_info should succeed for user-defined impl")
            .custom_kind
            .expect("custom_kind should be present for CoerceUnsized impl"),
        impl_source => {
...
mod tests {
    ...
    // =========================================================================
    // CoerceUnsizedInfo — pure struct tests (Part of #2217)
    //
    // CoerceUnsizedInfo derives Debug, Clone, PartialEq. We can test structural
    // properties even though constructing real Ty/TyStable requires rustc.
    // =========================================================================

    #[test]
    fn coerce_unsized_info_derives_are_present() {
        // Verify #[derive(Debug, Clone, PartialEq)] works on CoerceUnsizedInfo.
        // This is a compile-time test — if the derives are removed, this fails.
        fn assert_debug<T: std::fmt::Debug>() {}
        fn assert_clone<T: Clone>() {}
        fn assert_partial_eq<T: PartialEq>() {}
        assert_debug::<CoerceUnsizedInfo>();
        assert_clone::<CoerceUnsizedInfo>();
        assert_partial_eq::<CoerceUnsizedInfo>();
    }

    #[test]
    fn coercion_base_derives_are_present() {
        // CoercionBase derives Debug
        fn assert_debug<T: std::fmt::Debug>() {}
        assert_debug::<CoercionBase<'_>>();
    }

    // =========================================================================
    // CoerceUnsizedIterator — initial state (Part of #2217)
    //
    // The iterator's state machine is: src_ty/dst_ty start as Some, become None
    // after the final base-case iteration. We can't construct TyStable without
    // rustc, so we test the None-state behavior (documented contract).
    // =========================================================================

    // Note: We cannot test CoerceUnsizedIterator::next() without real Ty values
    // from rustc. The iterator's contract is:
    //   - First iteration: returns the starting (src_ty, dst_ty) pair
    //   - Last iteration: returns pointer types (base case)
    //   - After base case: returns None
    // This is documented in the iterator impl (lines 175-231) and tested via
    // compiletest integration tests that exercise unsized coercion paths.
}
