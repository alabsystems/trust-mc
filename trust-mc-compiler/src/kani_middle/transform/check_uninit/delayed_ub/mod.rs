...
#[derive(Debug, Clone)]
pub(crate) struct DelayedUbPass {
...
impl GlobalPass for DelayedUbPass {
    ...
    fn transform(
        ...
        let targets: HashSet<_> = instances
            .iter()
            .flat_map(|instance| {
                let body = instance.body().expect("instance should have body");
        ...
        if !targets.is_empty() {
            ...
            for entry_item in starting_items {
                ...
                if let Some(instance) = entry_fn {
                    let body = instance.body().expect("entry function should have body");
            ...
            for instance in instances {
                ...
                if instrumentation_added {
                    ...
                    transformer.cache.entry(instance).and_modify(|transformation_result| {
                        *transformation_result = TransformationResult(body);
                    });
                }
            }
        }
