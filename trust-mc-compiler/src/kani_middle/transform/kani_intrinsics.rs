...
#[derive(Debug, Clone)]
pub(crate) struct IntrinsicGeneratorPass {
    ...
    fn transform(&mut self, tcx: TyCtxt, body: Body, instance: Instance) -> (bool, Body) {
            ...
            attributes.fn_marker().and_then(|name| KaniIntrinsic::from_str(name.as_str()).ok())
        {
            match kani_intrinsic {
                ...
                // FloatToIntInRange is handled in AY codegen (Part of #1369).
                KaniIntrinsic::WriteAny
                | KaniIntrinsic::AnyModifies
                | KaniIntrinsic::AutomaticHarness
                | KaniIntrinsic::FloatToIntInRange => (false, body),
            }
        } else {
            (false, body)
        }
...
impl IntrinsicGeneratorPass {
    pub(crate) fn new(unsupported_check_type: CheckType, queries: &QueryDb) -> Self {
        ...
        IntrinsicGeneratorPass { unsupported_check_type, kani_defs, enable_uninit }
    }
    ...
    fn valid_value_body(&self, body: Body) -> Body {
        ...
        let Some(arg_ty) = new_body.locals().get(1).map(|local| local.ty) else {
            new_body.insert_check(
                &self.unsupported_check_type,
                &mut terminator,
                InsertPosition::Before,
                None,
                "trust_mc internal error: expected a pointer argument for valid value checks.",
            );
            return new_body.into();
        };
        let TyKind::RigidTy(RigidTy::RawPtr(target_ty, _)) = arg_ty.kind() else {
            let reason = format!(
                "trust_mc expected a raw pointer argument for valid value checks, found `{arg_ty}`."
            );
            new_body.insert_check(
                &self.unsupported_check_type,
                &mut terminator,
        ...
        match validity {
            ...
            Err(msg) => {
                ...
                let reason = format!(
                    "trust_mc currently doesn't support checking validity of `{target_ty}`. {msg}"
                );
                new_body.insert_check(
                    &self.unsupported_check_type,
                    &mut terminator,
    ...
    fn is_initialized_body(&mut self, body: Body) -> Body {
        ...
        let Some(arg_ty) = new_body.locals().get(1).map(|local| local.ty) else {
            new_body.insert_check(
                ...
                InsertPosition::Before,
                None,
                "trust_mc internal error: expected a pointer argument for memory initialization checks.",
            );
            return new_body.into();
        };
        ...
        let TyKind::RigidTy(RigidTy::RawPtr(pointee_ty, _)) = arg_ty.kind() else {
            let reason = format!(
                "trust_mc expected a raw pointer argument for memory initialization checks, found `{arg_ty}`."
            );
            new_body.insert_check(
                &self.unsupported_check_type,
                &mut source,
            ...
            Ok(pointee_info) => {
                match pointee_info.layout() {
                    PointeeLayout::Sized { layout } => {
                        ...
                        let is_ptr_initialized_instance = resolve_mem_init_fn(
                            *self
                                ...
                                .expect("IsPtrInitialized model should be defined"),
                            layout.len(),
                            *pointee_info.ty(),
                        );
                        let layout_operand =
                    ...
                    PointeeLayout::Slice { element_layout } => {
                        ...
                        let is_ptr_initialized_instance = resolve_mem_init_fn(
                            *self
                                .kani_defs
                                .get(&intrinsic)
                                .expect("kani intrinsic model should be defined"),
                            element_layout.len(),
                            slicee_ty,
                        );
                    ...
                    PointeeLayout::TraitObject => {
                        let reason: &str = "trust_mc does not support reasoning about memory initialization of pointers to trait objects.";
                    ...
                    PointeeLayout::Union { .. } => {
                        let reason: &str = "trust_mc does not yet support using initialization predicates on unions.";
    ...
    fn checked_size_of(&mut self, body: Body, instance: Instance) -> Body {
        ...
        if pointee_layout.is_sized() {
            ...
            let val_op = new_body.new_uint_operand(
                pointee_layout.size_of().expect("sized type should have known size") as _,
                UintTy::Usize,
                span,
            );
            ...
            let tail_ty = pointee_layout
                .unsized_tail()
                .expect("type with trait tail should have unsized tail");
            ...
            let elem_ty = pointee_layout
                .unsized_tail_elem_ty()
                .expect("type with slice tail should have element type");
            let elem_layout = LayoutOf::new(elem_ty);
            assert!(elem_layout.is_sized());

            let elem_size = new_body.new_uint_operand(
                elem_layout.size_of().expect("element layout should have known size") as _,
                UintTy::Usize,
                span,
            );
            let head_size =
            ...
            let align = new_body.new_uint_operand(
                pointee_layout.align_of().expect("pointee layout should have known alignment") as _,
                UintTy::Usize,
                span,
            );
            let ptr = Operand::Copy(Place::from(Local::from(1usize)));
    ...
    fn return_model(
        ...
        let def = self.kani_defs.get(&model.into()).expect("kani model should be defined");
        let size_of_dyn =
            Instance::resolve(*def, args).expect("kani model instance should be resolvable");
...
fn build_some(option: AdtDef, args: GenericArgs, val_op: Operand) -> Rvalue {
    let var_idx = option
        ...
        .expect("Option type should have a Some variant with fields");
...
fn build_none(option: AdtDef, args: GenericArgs) -> Rvalue {
    let var_idx = option
        .variants_iter()
        .find_map(|var| var.fields().is_empty().then_some(var.idx))
        .expect("Option type should have a None variant without fields");
