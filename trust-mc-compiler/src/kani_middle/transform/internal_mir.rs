...
pub(crate) trait RustcInternalMir {
    type T<'tcx>;
    ...
    fn internal_mir<'tcx>(&self, _tcx: TyCtxt<'tcx>) -> Self::T<'tcx> {
        match self {
            ...
            FakeReadCause::ForMatchedPlace(_opaque) => {
                // Part of #814: Opaque fields can't be converted back to internal rustc repr.
                // This path indicates stable MIR was modified in a way that loses internal state.
                unreachable!(
                    "FakeReadCause::ForMatchedPlace contains opaque data that cannot be \
                     converted back to rustc internal representation"
                )
            }
            ...
            FakeReadCause::ForLet(_opaque) => {
                // Part of #814: Opaque fields can't be converted back to internal rustc repr.
                unreachable!(
                    "FakeReadCause::ForLet contains opaque data that cannot be \
                     converted back to rustc internal representation"
                )
            }
...
impl RustcInternalMir for UserTypeProjection {
    ...
    fn internal_mir<'tcx>(&self, _tcx: TyCtxt<'tcx>) -> Self::T<'tcx> {
        // Part of #814: UserTypeProjection is fully opaque in stable MIR.
        // This path indicates a transform tried to convert back a user type annotation
        // that was constructed with opaque internal data.
        unreachable!(
            "UserTypeProjection is fully opaque and cannot be converted back to \
             rustc internal representation. This suggests a MIR transform issue."
        )
    }
}
    ...
    fn internal_mir<'tcx>(&self, tcx: TyCtxt<'tcx>) -> Self::T<'tcx> {
        match self {
            ...
            StatementKind::Coverage(_coverage_kind) => {
                // Part of #814: Coverage data is opaque in stable MIR.
                // Transforms should not create new Coverage statements.
                unreachable!(
                    "Coverage statement contains opaque data that cannot be converted back to \
                     rustc internal representation. Transforms should not create Coverage statements."
                )
            }
