...
//! diagnostic output across the `trust-mc-driver` components. Please follow the
...
#[cfg(test)]
thread_local! {
    static WARNING_TEST_MESSAGES: RefCell<Vec<String>> = const { RefCell::new(Vec::new()) };
    static WARNING_TEST_CAPTURE_ENABLED: Cell<bool> = const { Cell::new(false) };
}

/// Generate a valid crate name from the input file.
/// Note that this method will replace invalid characters from the crate name.
pub(crate) fn crate_name(path: &Path) -> Result<String> {
    let stem = path
        .file_stem()
        .context("input path should have a file stem")?
        .to_str()
        .context("input filename should be valid UTF-8")?;
    Ok(stem.replace(['-', '.'], "_"))
}
...
pub(crate) fn executable_basename(argv0: Option<&OsString>) -> Option<OsString> {
...
const DEFAULT_RUST_MIN_STACK_BYTES: &str = "16777216";

/// Ensure the compiler has enough stack for complex MIR patterns.
/// Only set when no explicit override exists.
pub(crate) fn apply_rust_min_stack(cmd: &mut Command) {
    if command_env_has_key(cmd, "RUST_MIN_STACK") {
        return;
    }
    if env::var_os("RUST_MIN_STACK").is_none() {
        cmd.env("RUST_MIN_STACK", DEFAULT_RUST_MIN_STACK_BYTES);
    }
}

fn command_env_has_key(cmd: &Command, key: &str) -> bool {
...
///
/// Accepts `&impl Display` so callers can pass `&format_args!(...)` to avoid
/// allocating a temporary `String` (compared to the `&format!(...)` pattern).
/// Existing `&format!(...)` callers continue to work without changes.
pub(crate) fn warning(msg: &(impl std::fmt::Display + ?Sized)) {
    #[cfg(test)]
    if WARNING_TEST_CAPTURE_ENABLED.with(Cell::get) {
        WARNING_TEST_MESSAGES.with(|messages| messages.borrow_mut().push(msg.to_string()));
    }
...
#[cfg(test)]
pub(crate) fn warning_test_messages_take() -> Vec<String> {
    WARNING_TEST_MESSAGES.with(|messages| std::mem::take(&mut *messages.borrow_mut()))
}

#[cfg(test)]
pub(crate) fn warning_test_capture_start() -> WarningTestCapture {
    WARNING_TEST_CAPTURE_ENABLED.with(|enabled| enabled.set(true));
    warning_test_messages_take();
    WarningTestCapture
}

#[cfg(test)]
pub(crate) struct WarningTestCapture;

#[cfg(test)]
impl Drop for WarningTestCapture {
    fn drop(&mut self) {
        warning_test_messages_take();
        WARNING_TEST_CAPTURE_ENABLED.with(|enabled| enabled.set(false));
    }
}

/// Print an error message. This will add an "error:" tag before the message and style accordingly.
pub(crate) fn error(msg: &(impl std::fmt::Display + ?Sized)) {
...
///
/// [`KaniArg`](args::KaniArg) -- First, there are trust_mc-specific arguments that the `trust-mc-compiler` uses to direct its codegen.
/// (See the [`Arguments`] struct in `trust-mc-compiler/src/args.rs` for how they are parsed.)
///
/// [`RustcArg`](args::RustcArg) -- Before codegen, the trust-mc compiler invokes `rustc` to compile Rust source to MIR,
/// and we pass arguments to influence that compilation.
/// See [`LibConfig`](crate::call_single_file::LibConfig) for how this is used in a typical trust-mc invocation.
///
/// [`CargoArg`](args::CargoArg) -- If we're calling trust-mc through `cargo trust-mc`, we also want to pass arguments to Cargo itself.
///
///
/// ## Usage
...
pub(crate) mod args {
    ...
    pub(crate) struct KaniArg(String);

    #[derive(Clone, PartialEq, Debug)]
    /// Arguments passed to `rustc`.
    pub(crate) struct RustcArg(OsString);

    #[derive(Clone, PartialEq)]
    /// Arguments passed to Cargo.
    pub(crate) struct CargoArg(OsString);
    ...
    pub(crate) fn encode_as_rustc_arg<'a>(
        kani_args: impl IntoIterator<Item = &'a KaniArg>,
    ) -> RustcArg {
        format!(
            r#"-Cllvm-args={}"#,
            kani_args
...
mod tests {
    ...
    #[test]
    fn check_apply_rust_min_stack() {
        // Case 1: Sets RUST_MIN_STACK when not present
        let mut cmd = Command::new("test");
        apply_rust_min_stack(&mut cmd);
        let has_stack = cmd.get_envs().any(|(k, v)| {
            k == OsStr::new("RUST_MIN_STACK") && v == Some(OsStr::new(DEFAULT_RUST_MIN_STACK_BYTES))
        });
        assert!(has_stack, "RUST_MIN_STACK should be set to default value");

        // Case 2: Doesn't override if already set in command
        let mut cmd2 = Command::new("test");
        cmd2.env("RUST_MIN_STACK", "1234567");
        apply_rust_min_stack(&mut cmd2);
        let has_original = cmd2
            .get_envs()
            .any(|(k, v)| k == OsStr::new("RUST_MIN_STACK") && v == Some(OsStr::new("1234567")));
        assert!(has_original, "RUST_MIN_STACK should keep original value");
    }

    #[test]
    fn warning_capture_is_opt_in() {
        warning_test_messages_take();
        warning("not captured");
        assert!(warning_test_messages_take().is_empty());
    }

    #[test]
    fn warning_capture_guard_clears_and_disables_capture() {
        {
            let _capture = warning_test_capture_start();
            warning("captured warning");
            assert_eq!(warning_test_messages_take(), vec![String::from("captured warning")]);
        }
        warning("after capture");
        assert!(warning_test_messages_take().is_empty());
    }
}
