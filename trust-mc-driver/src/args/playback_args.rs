...
#[derive(Debug, Parser)]
pub(crate) struct CargoPlaybackArgs {
    #[command(flatten)]
...
#[derive(Debug, Parser)]
pub(crate) struct KaniPlaybackArgs {
...
pub(crate) struct PlaybackArgs {
    /// Common args always available to trust-mc subcommands.
    #[command(flatten)]
...
mod tests {
    ...
    fn check_cargo_parse_test_works() {
        ...
        assert_eq!(args.playback.common_opts.message_format, MessageFormat::Human);
    }
    ...
    fn check_parse_format_works() {
        ...
        let args = CargoPlaybackArgs::try_parse_from(input).unwrap();
        args.validate().unwrap();
        assert_eq!(args.playback.common_opts.message_format, MessageFormat::Json);
    }
    ...
    #[test]
    fn check_kani_parse_format_works() {
        let input =
            "playback -Z concrete-playback input.rs --message-format=json".split_whitespace();
        let args = KaniPlaybackArgs::try_parse_from(input).unwrap();
        // Don't validate this since we check if the input file exists.
        assert_eq!(args.playback.common_opts.message_format, MessageFormat::Json);
    }
