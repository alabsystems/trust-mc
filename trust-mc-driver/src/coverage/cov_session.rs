...
impl KaniSession {
    ...
    pub(crate) fn save_coverage_metadata(&self, project: &Project, stamp: &str) -> Result<()> {
    ...
    fn save_coverage_metadata_cargo(&self, project: &Project, stamp: &str) -> Result<()> {
        let build_target = env!("TARGET");
        let metadata = self.cargo_metadata(build_target)?;
        let default_dir: PathBuf = metadata.target_directory.into();
        let target_dir = self.args.target_dir.as_ref().unwrap_or(&default_dir).join("kani");
    ...
    fn save_coverage_metadata_standalone(&self, project: &Project, stamp: &str) -> Result<()> {
        let input = project
            .input
            .clone()
            .context("standalone mode requires input file")?
            .canonicalize()
            .context("failed to canonicalize input path")?;
        let input_dir = input.parent().context("input file has no parent directory")?.to_path_buf();
    ...
    pub(crate) fn save_coverage_results(
        &self,
        project: &Project,
    ...
    pub(crate) fn save_coverage_results_cargo(
        &self,
        results: &Vec<HarnessResult>,
        stamp: &str,
    ) -> Result<()> {
        ...
        for harness_res in results {
            let harness_name = &harness_res.harness.mangled_name;
            ...
            let cov_results = harness_res
                .result
                .coverage_results
                .as_ref()
                .context("harness missing coverage results")?;
    ...
    pub(crate) fn save_coverage_results_standalone(
        &self,
        project: &Project,
        results: &Vec<HarnessResult>,
        stamp: &str,
