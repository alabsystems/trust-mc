...
//! Module for parsing concrete values from verification output traces,
...
impl KaniSession {
    ...
    pub(crate) fn gen_and_add_concrete_playback(
        &self,
        ...
        let harness_values = extract_harness_values(&verification_result.results);
        ...
        if harness_values.is_empty() {
            ...
            match playback_mode {
                ConcretePlaybackMode::Print => {
                    for generated_unit_test in &unit_tests {
                        println!(
    ...
    fn add_tests_inplace(
        ...
        let source_string = read_to_string(source_path)
        ...
        if !unit_tests.is_empty() {
            ...
            for line in source_reader.lines().map_while(Result::ok) {
                ...
                if curr_line_num == proof_harness_end_line {
                    for unit_test in &unit_tests {
                        ...
                        for unit_test_line in &unit_test.code {
                            curr_line_num += 1;
            ...
            temp_file
                .persist(source_path)
                .with_context(|| format!("failed to persist temp file to {source_path}"))?;
        }
    ...
    fn run_rustfmt(
        ...
        // Use timeout-protected versions to prevent rustfmt hangs (#999)
        if self.args.common_args.quiet {
            self.run_suppress_with_tool_timeout(cmd)
                .context("Failed to rustfmt modified source code.")?;
        } else {
            self.run_terminal_with_tool_timeout(cmd)
                .context("Failed to rustfmt modified source code")?;
        }
        Ok(())
    }
...
fn extract_parent_dir_and_src_file(src_path: &Path) -> Result<(String, String)> {
    let parent_dir = src_path.parent().map(|p| p.to_string_lossy().to_string()).unwrap_or_default();
    let src_file = src_path
        .file_name()
        .map(|n| n.to_string_lossy().to_string())
        .ok_or_else(|| anyhow::anyhow!("source path has no file name: {}", src_path.display()))?;
...
/// Extract concrete values from the verification output processed items.
...
mod concrete_vals_extractor {
    use crate::property_model::{CheckStatus, Property, TraceItem, TraceValue};

    #[derive(Hash)]
    pub(super) enum ConcreteItem {
    ...
    #[derive(Hash)]
    pub(super) struct PrimitiveConcreteVal {
        pub(super) byte_arr: Vec<u8>,
        pub(super) interp_val: String,
    }
    ...
    pub(super) fn extract_harness_values(
        ...
        result_items
            ...
            .filter_map(|property| {
                ...
                let trace = match property.trace.as_ref() {
                    Some(t) => t,
                    None => {
                        tracing::warn!("Missing trace for {}", property.property_name());
                        return None;
                    }
                };
    ...
    fn extract_primitive_value(value: &TraceValue) -> Option<PrimitiveConcreteVal> {
        ...
        // Reverse because of endianness of verification trace.
        for i in (0..declared_width).step_by(8).rev() {
            let str_chunk = &bit_concrete_val[i..i + 8];
            let str_chunk_len = str_chunk.len();
            if str_chunk_len != 8 {
                tracing::warn!(
                    "Tried to read a chunk of 8 bits but actually read {str_chunk_len} bits"
                );
                return None;
            }
            let next_byte = match u8::from_str_radix(str_chunk, 2) {
                Ok(b) => b,
                Err(_) => {
                    tracing::warn!("Couldn't convert the string chunk `{str_chunk}` to u8");
                    return None;
                }
            };
        ...
        // In ARM64 Linux, the verification trace may produce a character instead of
        // a number for interpreted values because the char type is unsigned on that
        // platform. For example, for the value `101` it will produce `'e'` instead
        // of `101`. To correct this, we check if the value starts and ends with `'`,
        // and convert the character into its ASCII value in that case.
        let interp_val = {
            let interp_val_str = interp_concrete_val.to_string();
            if interp_val_str.starts_with('\'')
                && interp_val_str.ends_with('\'')
                && interp_val_str.len() >= 3
            {
                let interp_num = interp_val_str.chars().nth(1)? as u8;
    ...
    fn extract_from_trace_item(trace_item: &TraceItem) -> Option<ConcreteItem> {
        ...
        if let Some(array_elements) = &value.elements {
            // Every array element must be extractable as a primitive. If any fails,
            // the array is malformed so we skip the entire item rather than generating
            // a too-short array.
            let concrete_vals: Option<Vec<_>> = array_elements
                .iter()
                .map(|array_value| {
                    extract_primitive_value(&array_value.value).or_else(|| {
                        tracing::warn!("Couldn't extract concrete value from {array_value:?}");
                        None
                    })
                })
                .collect();
            return concrete_vals.map(ConcreteItem::Array);
        // If the array length is <= 64, the verification trace includes TraceItems for each primitive value of the array,
        // as well as the `elements` field with the entire array (for arrays length > 65, it just has `elements`).
        // Filter out any instance of any_raw_array to avoid generating duplicate values for the primitive values that are separate from `elements`.
...
mod tests {
    ...
    use crate::property_model::{
        CheckStatus, Property, PropertyId, RawSourceLocation, TraceArrayValue, TraceData,
        TraceItem, TraceValue,
    };
    ...
    /// Test that AY-style traces work with concrete value extraction.
    /// AY uses function name "kani::any_raw_internal::<ay>" and binary format for data.
    #[test]
    fn check_concrete_vals_extractor_ay_style() {
        let processed_items = [Property {
            description: Cow::Borrowed("assertion failed"),
            property_id: PropertyId {
                fn_name: Some("test_harness".to_string()),
                class: Cow::Borrowed("assertion"),
                id: 0,
            },
            status: CheckStatus::Failure,
            ...
            trace: Some(vec![TraceItem {
                ...
                source_location: Some(RawSourceLocation {
                    column: None,
                    file: None,
                    // AY-style function name
                    function: Some("kani::any_raw_internal::<ay>".to_string()),
                    line: None,
                }),
                value: Some(TraceValue {
                    // AY uses 8-bit binary strings
                    binary: Some("01100101".to_string()),
                    // AY uses binary or hex format for data
                    data: Some(TraceData::NonBool("#b01100101".to_string())),
                    width: Some(8),
                    elements: None,
                }),
            }]),
        ...
        if let ConcreteItem::Primitive(concrete_val) = concrete_item {
            ...
            assert_eq!(concrete_val.byte_arr, vec![101]);
            // AY-style interp_val preserves the binary format
            assert_eq!(concrete_val.interp_val, "#b01100101");
        }
    }

    /// Test that AY-style multi-byte traces work correctly.
    /// Verifies endianness handling for multi-byte traces.
    #[test]
    fn check_concrete_vals_extractor_ay_style_multibyte() {
        let processed_items = [Property {
            description: Cow::Borrowed("assertion failed"),
            property_id: PropertyId {
                fn_name: Some("test_harness".to_string()),
            ...
            trace: Some(vec![TraceItem {
                ...
                value: Some(TraceValue {
                    // 16-bit value: 0x0181 = 385 decimal
                    // Binary split: 00000001 (byte 0) + 10000001 (byte 1)
                    // Extraction reverses for endianness: [129, 1]
                    binary: Some("0000000110000001".to_string()),
                    // AY uses hex format for data field (for comment in generated test)
                    data: Some(TraceData::NonBool("#x0181".to_string())),
                    width: Some(16),
                    elements: None,
                }),
            }]),
        ...
        assert!(matches!(concrete_item, ConcreteItem::Primitive(_)));
        if let ConcreteItem::Primitive(concrete_val) = concrete_item {
            // Compare: vec![1, 3] from "0000001100000001" in the endianness test above
            // But our binary is "0000000110000001" (0x0181)
            // High byte: 00000001 = 1, Low byte: 10000001 = 129
            // Reversed for little-endian: [129, 1]
            assert_eq!(concrete_val.byte_arr, vec![129, 1]);
            assert_eq!(concrete_val.interp_val, "#x0181");
        }
    }
}
