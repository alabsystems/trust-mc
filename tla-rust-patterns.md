...
implementation code. It mirrors an earlier Python-focused TLA+ correspondence
guide, but uses the ay project as the reference implementation for Rust.

## Reference Implementation

Use ay's `ay-tla-bridge` crate as the baseline for how to run TLC from Rust and
interpret outcomes. In the ay repo, see:

- `crates/ay-tla-bridge/src/lib.rs`
...
Each Rust repo with TLA+ specs should maintain a `TLA_TO_RUST_MAPPING.md`
...
The canonical example is the `TLA_TO_RUST_MAPPING.md` maintained in the
[ay repo](https://github.com/alabsystems/ay).
