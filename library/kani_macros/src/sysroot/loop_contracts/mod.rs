...
pub(crate) struct ForLoopExtraStmts {
    init_iter_stmt: Stmt,
    init_len_stmt: Stmt,
...
pub(crate) fn transform_for_to_loop(
...
pub(crate) fn loop_invariant(attr: TokenStream, item: TokenStream) -> TokenStream {
...
pub(crate) fn loop_modifies(attr: TokenStream, item: TokenStream) -> TokenStream {
...
pub(crate) fn loop_decreases(attr: TokenStream, item: TokenStream) -> TokenStream {
    // A comma-separated (multi-dimensional) or otherwise malformed measure fails
    // to parse as a single `Expr`. Emit a clean compile error rather than
    // panicking on `.unwrap()`, which surfaces as a "custom attribute panicked"
    // proc-macro ICE (exit 101) instead of a diagnostic. A bare `compile_error!`
    // (prelude macro) is used rather than `abort_call_site!` (needs the entry
    // point annotated `#[proc_macro_error]`) or `syn::Error::to_compile_error`
    // (emits `::core::compile_error!`, which fails to resolve against the custom
    // verification sysroot).
    let decreases_expr: Expr = match syn::parse(attr.clone()) {
        Ok(expr) => expr,
        Err(err) => {
            // Multi-dimensional (comma-separated) decreases: parse the list and
            // wrap it as a TUPLE expression so the file COMPILES. The transform
            // layer's non-identity-measure fail-closed lane then emits a failing
            // decreases ranking check — matching Kani, whose CBMC lowering
            // performs no lexicographic comparison on tuple decreases
            // (kani#3168) and FAILS the check. Do NOT implement true
            // lexicographic ranking while the corpus oracle encodes Kani's
            // broken behavior — a correct proof would read as a missed bug.
            use syn::Token;
            use syn::parse::Parser;
            use syn::punctuated::Punctuated;
            match Punctuated::<Expr, Token![,]>::parse_terminated.parse(attr) {
                Ok(list) if list.len() > 1 => {
                    let elems = list.into_iter();
                    syn::parse_quote!(( #(#elems),* ))
                }
                _ => {
                    let span = err.span();
                    let msg = "`#[kani::loop_decreases]` expects a measure expression.";
    ...
    let loop_stmt: Stmt = syn::parse(item.clone()).unwrap();
    let original_span = decreases_expr.span();

    match &loop_stmt {
        Stmt::Expr(Expr::While(_) | Expr::Loop(_) | Expr::ForLoop(_), _) => {}
        _ => abort_call_site!("`#[kani::loop_decreases]` is only supported on loops.";
            note = "place `#[kani::loop_decreases(...)]` directly on a while, loop, or for expression.";
        ),
    }

    let loop_id = generate_unique_id_from_span(&loop_stmt);
    let register_ident = format_ident!("kani_register_loop_decreases{}", loop_id);

    quote_spanned!(original_span =>
    {
        #[inline(never)]
        #[kanitool::fn_marker = "kani_register_loop_decreases"]
        const fn #register_ident<T, F: Fn() -> T>(_f: &F, _transformed: usize) -> bool {
            true
        }
        let _ = #register_ident(&|| { #decreases_expr }, 0);
        #loop_stmt
    })
    .into()
}
