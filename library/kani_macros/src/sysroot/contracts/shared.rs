...
pub(crate) fn split_for_remembers(
    stmts: &[Stmt],
...
pub(crate) fn try_as_result_assign(stmt: &syn::Stmt) -> Option<&syn::LocalInit> {
...
pub(crate) fn build_ensures(data: &ExprClosure) -> (TokenStream2, Expr) {
    ...
    // NOTE: bind by MOVE (no `.clone()`), matching upstream Kani. The clone
    // auto-deref'd `old(ptr)` on `&mut u32` to a `u32` value (u32::clone via
    // deref), so the source's outer `*old(ptr)` became `*u32` -> E0614. Kani's
    // move-binding lets rustc surface the NATURAL diagnostic for non-Copy
    // old() args ("use of moved value") — exactly what the history/ui
    // expected files encode (noncopy_ignore).
