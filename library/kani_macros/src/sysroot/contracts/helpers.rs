...
pub(crate) fn return_type_to_type(return_type: &syn::ReturnType) -> Cow<'_, syn::Type> {
    match return_type {
...
pub(crate) fn pat_to_bindings(pat: &syn::Pat) -> Vec<(MutBinding, &Ident)> {
    use syn::Pat;
    let mk_err = |typ| {
        pat.span()
            .unwrap()
...
pub(crate) fn find_contract_closure<'a>(
    stmts: &'a mut [Stmt],
    name: &'static str,
) -> Option<&'a mut Stmt> {
...
pub(crate) fn expect_closure<'a>(stmts: &'a mut [Stmt], name: &'static str) -> &'a mut Stmt {
...
///
/// Panic if no closure was found.
pub(crate) fn expect_closure_in_match<'a>(
    stmts: &'a mut [Stmt],
    name: &'static str,
) -> &'a mut Stmt {
...
pub(crate) fn closure_body(closure: &mut Stmt) -> &mut ExprBlock {
...
pub(crate) fn matches_path<E>(path: &syn::Path, mtch: &[E]) -> bool
where
...
pub(crate) fn is_token_stream_2_comma(t: &proc_macro2::TokenTree) -> bool {
    matches!(t, proc_macro2::TokenTree::Punct(p) if p.as_char() == ',')
}

pub(crate) fn chunks_by<'a, T, C: Default + Extend<T>>(
...
/// Find an if expression by traversing nested blocks and statements.
///
/// This handles cases where the if expression may be wrapped in additional
/// blocks or nested within other statements. Part of #693.
pub(crate) fn find_if_expr_mut(stmts: &mut [Stmt]) -> Option<&mut syn::ExprIf> {
    for stmt in stmts.iter_mut() {
        match stmt {
            // Check direct if expressions at this level
            Stmt::Expr(Expr::If(if_expr), ..) => {
                return Some(if_expr);
            }
            // Traverse into nested blocks
            Stmt::Expr(Expr::Block(block_expr), ..) => {
                if let Some(found) = find_if_expr_mut(&mut block_expr.block.stmts) {
                    return Some(found);
                }
            }
            // Traverse into local initializers
            Stmt::Local(Local { init: Some(LocalInit { expr, .. }), .. }) => {
                match expr.as_mut() {
                    // Check if the local initializer is an if expression
                    Expr::If(if_expr) => {
                        return Some(if_expr);
                    }
                    // Traverse into block initializers
