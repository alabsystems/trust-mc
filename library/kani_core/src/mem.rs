...
// ## Memory Model Hooks
//
// Functions in this module use marker hooks that the AY backend intercepts:
//
// | Function           | Hook               | AY Implementation                                            |
// |--------------------|--------------------|--------------------------------------------------------------|
// | `pointer_object`   | PointerObjectHook  | `ptr / HEAP_STRIDE` (allocation ID)                          |
// | `pointer_offset`   | PointerOffsetHook  | `ptr % HEAP_STRIDE` (offset within allocation)               |
// | `is_allocated`     | IsAllocatedHook    | `obj_valid[ptr_obj] && same_obj(ptr, ptr+size-1)`            |
// | `same_allocation`  | (uses above)       | `pointer_object(ptr1) == pointer_object(ptr2)`               |
//
// The AY heap model: `Ptr = bitvec(64)`, `Heap = Map<PtrId, Object>`
// See: trust_mc-compiler/src/codegen_ay/context.rs::heap_is_allocated() for implementation
// AY heap allocation model (#1100)
...
macro_rules! kani_mem {
    ($core:tt) => {
        ...
        pub(super) fn same_allocation_internal<T: PointeeSized>(
            ...
            internal::same_allocation(addr1, addr2)
        }
        ...
        /// Note: Could be optimized for sized T with a compile-time size_of.
        ...
        pub fn is_inbounds<T: MetaSized>(ptr: *const T) -> bool {
            ...
            if sz == 0 {
                ...
                if !unsafe { is_allocated(data_ptr, 0) } {
                    // Part of #3212: unallocated pointers are not dereferenceable.
                    // Previous `kani::unsupported()` emitted unconditional error rules
                    // in CHC/BMC, causing spurious CTREX when the solver explored this
                    // branch despite heap model constraints.
                    return false;
                }
        ...
        /// Check if the pointer `_ptr` points to an allocated region of at least `_size` bytes.
        ///
        /// AY: checks `obj_valid[ptr / HEAP_STRIDE]` and verifies the end pointer
        /// maps to the same allocation object.
        ///
        /// # Safety
        ///
        ...
        /// Internal memory model helpers.
        ///
        /// These functions use marker hooks (`pointer_object`, `is_allocated`)
        /// that the AY backend intercepts via `codegen_kani_call.rs`.
        pub(super) mod internal {
            use super::*;
            /// Compare allocation IDs using `pointer_object` and check bounds with `is_allocated`.
            pub fn same_allocation(addr1: *const (), addr2: *const ()) -> bool {
                ...
                (obj1 == crate::kani::mem::pointer_object(addr2)) && {
                    if !unsafe { is_allocated(addr1, 0) || is_allocated(addr2, 0) } {
                        // Part of #3212: neither pointer is allocated, so they
                        // cannot be in the same allocation. See Change 1 above.
                        return false;
                    }
        ...
        /// Get the allocation object ID for the given pointer.
        ///
        /// AY: Returns `ptr / HEAP_STRIDE` (allocation ID from heap model, #1410).
        #[doc(hidden)]
        ...
        /// Get the byte offset within the allocation for the given pointer.
        ///
        /// AY: Returns `ptr % HEAP_STRIDE` (byte offset from heap model, #1410).
        #[doc(hidden)]
