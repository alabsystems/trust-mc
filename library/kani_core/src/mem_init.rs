...
macro_rules! kani_mem_init {
    ($core:path) => {
        // SAFETY: Thread-safety of mutable statics
        //
        // These `static mut` variables are safe because Kani verification executes in a
        // single-threaded context. The symbolic execution model explores all possible
        // interleavings but does not actually run concurrent threads:
        //
        // 1. **No data races**: Only one logical thread of execution accesses these statics
        //    at any point in the verification. Kani's model checker serializes all operations.
        //
        // 2. **No aliasing violations**: All accesses go through the defined accessor functions
        //    (`get`, `set`, `copy`, `bless`) which take `&mut self`, ensuring exclusive access
        //    at the Rust type-system level within each verification path.
        //
        // 3. **Verification context only**: This code is not intended for production runtime use.
        //    It instruments code for static analysis by the Kani verifier, which operates on
        //    a single-threaded abstract execution model.
        //
        // If Kani's execution model changes to support true concurrency, these statics would
        // need to be replaced with thread-safe alternatives (e.g., `std::sync::Mutex` or
