...
uint8_t *__rust_alloc(size_t size, size_t align)
{
    ...
    // Soundness note (alignment): We assert alignment is valid but malloc() doesn't
    // guarantee the requested alignment. This matches CBMC's approach which assumes
    // MALLOC_ALIGNMENT = MAX_INT (effectively ignoring alignment constraints).
    // Programs relying on strict alignment (>16 bytes) may not be correctly modeled.
    // Upstream: https://github.com/model-checking/kani/issues/1168
...
uint8_t *__rust_alloc_zeroed(size_t size, size_t align)
{
    ...
    // Soundness note: See __rust_alloc for alignment handling rationale.
    // Upstream: https://github.com/model-checking/kani/issues/1168
    __KANI_assert(__KANI_is_nonzero_power_of_two(align), "Alignment is power of two");
