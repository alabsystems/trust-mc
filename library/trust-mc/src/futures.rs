...
//! Support for verifying async Rust code with futures and async/await.
//!
//! This module provides executors and utilities for verifying async Rust code
//! in trust_mc. Since trust_mc cannot use standard async runtimes like `tokio` or
//! `async-std`, this module provides simplified alternatives.
//!
//! # Overview
//!
//! | Function | Spawning | Use Case |
//! |----------|----------|----------|
//! | [`block_on`] | No | Simple async code without spawning |
//! | [`block_on_with_spawn`] | Yes | Concurrent tasks with [`spawn`] |
//!
//! # Basic Usage
//!
//! For simple async code that doesn't spawn additional tasks:
//!
//! ```rust
//! use kani::futures::block_on;
//!
...
//!
//! #[kani::proof]
//! fn verify_async_function() {
//!     let result = block_on(fetch_data());
//!     kani::assert(result == 42, "Expected 42");
//! }
//! ```
//!
//! # Concurrent Tasks with Spawn
//!
//! For code that spawns concurrent tasks, use [`block_on_with_spawn`] with a
//! scheduling strategy:
//!
//! ```rust
//! use kani::futures::{block_on_with_spawn, spawn, RoundRobin};
//!
//! #[kani::proof]
//! fn verify_concurrent_tasks() {
//!     block_on_with_spawn(
//!         async {
//!             let handle1 = spawn(async { /* task 1 */ });
//!             let handle2 = spawn(async { /* task 2 */ });
//!             handle1.await;
//!             handle2.await;
//!         },
//!         RoundRobin::default(),
...
//!
//! The [`SchedulingStrategy`] trait controls task execution order:
//!
//! - [`RoundRobin`] - Deterministic, cycles through tasks in order (faster)
//! - Custom strategies can explore nondeterministic orderings for concurrency bugs
//!
//! # Limitations
//!
//! - No I/O operations (network, file, timers)
//! - No real parallelism - tasks run sequentially under verification
//! - Wakers are ignored - the executor busy-polls until completion
//!
//! # Unstable Feature
//!
//! These APIs require the `async-lib` unstable feature:
//! ```text
//! cargo trust_mc -Z async-lib
//! ```
...
///
/// This is intended as a drop-in replacement for `futures::block_on`, which trust_mc cannot handle.
/// Whereas a clever executor like `block_on` in `futures` or `tokio` would interact with the OS scheduler
/// to be woken up when a resource becomes available, this is not supported by trust_mc.
...
///
/// If your harness spawns several tasks, trust_mc's scheduler has to decide in what order to poll them.
...
///
/// Nondeterministic scheduling strategies can be very slow to verify because they require trust_mc to check a large number of permutations of tasks.
...
pub trait SchedulingStrategy {
    ...
    /// For example, if pick_task(4) returns (2, CanAssumeRunning) than it picked the task with index 2 and allows trust_mc to `assume` that this task is still running.
...
#[allow(clippy::unwrap_used)] // upstream Kani: unwrap after is_none guard
impl Future for JoinHandle {
    ...
    fn poll(self: Pin<&mut Self>, cx: &mut Context<'_>) -> std::task::Poll<Self::Output> {
        if unsafe { GLOBAL_EXECUTOR.as_mut().unwrap().tasks[self.index].is_some() } {
            ...
            cx.waker().wake_by_ref(); // For completeness. But trust_mc currently ignores wakers.
            std::task::Poll::Ready(())
        }
    }
}
...
pub fn spawn<F: Future<Output = ()> + Sync + 'static>(fut: F) -> JoinHandle {
    unsafe {
        if let Some(executor) = GLOBAL_EXECUTOR.as_mut() {
            ...
            // An explicit panic instead of `.expect(...)` has better location information in trust_mc's output
...
#[crate::unstable(feature = "async-lib", issue = 2559, reason = "experimental async support")]
#[allow(static_mut_refs)]
#[allow(clippy::unwrap_used)] // upstream Kani: unwrap after is_none assertion
