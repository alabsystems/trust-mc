...
/// Run a generated concrete-playback test in playback mode.
///
/// The concrete values are produced by the trust_mc driver and replayed into the
/// proof harness in the order they were observed during verification.
/// Verification builds keep this helper nameable for compile compatibility,
/// but executing it outside playback mode is unsupported.
pub fn concrete_playback_run<F: Fn()>(mut local_concrete_vals: Vec<Vec<u8>>, proof_harness: F) {
    ...
    CONCRETE_VALS.with(|glob_concrete_vals| {
        ...
        assert!(
            ...
            2) There's a bug in trust_mc. Please report the issue here: <https://github.com/alabsystems/trust_mc/issues/new/choose>",
            ref_glob_concrete_vals
        );
    });
}

/// Iterate over `any_raw_internal` since the verifier produces assignment per element.
...
/// kani::any_raw_internal. Zero-Sized Types are defaulted to an empty vector.
///
/// # Safety
///
