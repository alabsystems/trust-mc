...
pub enum Mode {
    ...
    CargoCoverage,
    CargoTrustMc,
    CargoTrustMcTest, // `cargo trust_mc --tests`.
    CoverageBased,
    Exec,
    Expected,
    Stub,
...
impl FromStr for Mode {
    ...
    fn from_str(s: &str) -> Result<Mode, ()> {
        match s {
            "trust_mc" => Ok(TrustMc),
            "cargo-trust_mc" => Ok(CargoTrustMc),
            "cargo-trust_mc-test" => Ok(CargoTrustMcTest),
...
impl fmt::Display for Mode {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let s = match *self {
            ...
            CargoTrustMc => "cargo-trust_mc",
            CargoTrustMcTest => "cargo-trust_mc-test",
...
pub struct Config {
    ...
    /// Extra arguments to be passed to trust_mc in this regression.
    /// Note that there is no validation done whether these flags conflict with existing flags.
    /// For example, one could add `--trust_mc-flag=--only-codegen` to only compile all tests.
