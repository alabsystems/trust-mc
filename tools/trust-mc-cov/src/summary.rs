...
pub(crate) fn summary_main(args: &SummaryArgs) -> Result<()> {
...
/// validation.
pub(crate) fn validate_summary_args(_args: &SummaryArgs) -> Result<()> {
...
pub(crate) fn line_coverage_results(
    info: &FunctionInfo,
...
fn print_coverage_markdown_info(info: &Vec<FileCoverageInfo>) {
    ...
    for cov_info in info {
        let filename = cov_info.filename.clone();
