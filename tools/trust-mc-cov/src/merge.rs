...
pub(crate) fn merge_main(args: &MergeArgs) -> Result<()> {
...
/// validation.
pub(crate) fn validate_merge_args(_args: &MergeArgs) -> Result<()> {
...
fn combine_raw_results(results: &Vec<CoverageResults>) -> CombinedCoverageResults {
    ...
    for (file_name, fun_name) in all_file_function_names {
        ...
        for result in results {
            if let Some(checks) = result.data.get(&file_name) {
                this_fun_checks.extend(checks.iter().filter(|check| check.function == fun_name))
            }
        }
        ...
        new_data.entry(file_name.clone()).or_default().push((fun_name, new_results));
    }
