...
// SPDX-License-Identifier: Apache-2.0 OR MIT
//! Unified handling of unstable-feature flags across trust_mc.
//!
...
//!
//! ```
//! # use trust_mc_metadata::unstable::*;
//! use clap::Parser;
//!
//! #[derive(Parser)]
//! struct MyCmdArgs {
...
pub enum UnstableFeature {
    /// Enable array iteration unrolling transformation (#468).
    /// Transforms `for x in array` loops to indexed loops, bypassing iterator infrastructure.
    ArrayIterUnroll,
    /// Enable range iteration unrolling transformation (#1534).
    /// Transforms `for i in 0..n` loops to explicit index bounds, bypassing Option-based iterator.
    RangeIterUnroll,
    /// Enable trust_mc's unstable async library.
    AsyncLib,
    /// Enable the autoharness subcommand.
    Autoharness,
    /// Enable concrete playback flow.
    ConcretePlayback,
    /// Allow trust_mc to link against C code.
    CFfi,
    /// trust_mc APIs related to floating-point operations (e.g. `float_to_int_in_range`)
    FloatLib,
    ...
    Stubbing,
    /// Symbolic collection models (e.g. TrustMcMap).
    /// Current support relies on verifier interception in CHC mode.
    SymbolicCollections,
...
impl UnstableFeature {
    ...
    pub(crate) fn as_argument(&self) -> [&str; 2] {
        ["-Z", self.as_ref()]
    }
...
impl EnabledUnstableFeatures {
    ...
    pub fn as_arguments(&self) -> impl Iterator<Item = &str> {
        self.enabled_unstable_features.iter().flat_map(UnstableFeature::as_argument)
    }
