...
python3 "${ROOT_DIR}/scripts/check_portable_manifests.py" "${ROOT_DIR}"
...
require_frozen_content_checkout() {
    ...
    fetched_main="$(python3 "${ROOT_DIR}/scripts/check_live_main.py" \
...
trust_ir_audit="$(python3 "${ROOT_DIR}/scripts/check_first_party_git_pins.py" \
...
for package in trust-ir trust-ir-build; do
    resolved_id="$(cargo pkgid --frozen --manifest-path "${ROOT_DIR}/Cargo.toml" "${package}")" \
        || die "Cargo could not resolve ${package}"
    checkout_id="$(cargo pkgid --frozen --manifest-path \
...
clean_audit="$(python3 "${ROOT_DIR}/scripts/check_first_party_git_pins.py" \
...
resolved_clean="$(cargo pkgid --frozen --manifest-path "${ROOT_DIR}/Cargo.toml" \
...
trust_vc_audit="$(python3 "${ROOT_DIR}/scripts/check_first_party_git_pins.py" \
...
resolved_vc="$(cargo pkgid --frozen --manifest-path "${ROOT_DIR}/Cargo.toml" \
