...
KANI_STANDALONE_OUTPUT_HEAD_PRODUCT=`echo ${KANI_STANDALONE_OUTPUT_HEAD} | awk '{print $1,$2,$3}'`
KANI_STANDALONE_OUTPUT_HEAD_VERSION=`echo ${KANI_STANDALONE_OUTPUT_HEAD} | awk '{print $4}'`

if [[ $KANI_VERSION_CMD_NAME == "kani" ]]; then
    echo "success: legacy version command name appears"
else
    echo "failed: expected legacy version command name"
    exit 1
fi

if [[ $KANI_STANDALONE_OUTPUT_HEAD_PRODUCT == "Kani Rust Verifier" ]]; then
    echo "success: legacy verifier product appears"
else
    echo "failed: expected legacy verifier product"
    exit 1
fi
