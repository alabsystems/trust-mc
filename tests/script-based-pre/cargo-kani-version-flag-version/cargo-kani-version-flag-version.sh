...
KANI_VERSION_CMD_NAME=`echo ${KANI_VERSION_CMD} | awk '{print $1}'`
...
KANI_CARGO_OUTPUT_HEAD_PRODUCT=`echo ${KANI_CARGO_OUTPUT_HEAD} | awk '{print $1,$2,$3}'`
KANI_CARGO_OUTPUT_HEAD_VERSION=`echo ${KANI_CARGO_OUTPUT_HEAD} | awk '{print $4}'`

if [[ $KANI_VERSION_CMD_NAME == "cargo-kani" ]]; then
    echo "success: legacy cargo version command name appears"
else
    echo "failed: expected legacy cargo version command name"
    exit 1
fi

if [[ $KANI_CARGO_OUTPUT_HEAD_PRODUCT == "Kani Rust Verifier" ]]; then
    echo "success: legacy cargo verifier product appears"
else
    echo "failed: expected legacy cargo verifier product"
    exit 1
fi
