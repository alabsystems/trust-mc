...
//!
//! Note that the verifier's offset logic will wrap around the object bits, not the entire address space, when
...
fn original_harness() {
    ...
    // Without UB detection, the offsets are the same, but pointer arithmetic
