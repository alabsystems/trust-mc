...
//! Additional active regressions for potentially uninitialized union access.
//! Until AY models union initialization precisely, both initialized and
//! padding cases must fail closed with the explicit diagnostics in
//! `unions_uninit_unsupported.expected`.
...
unsafe fn pointer_union_should_pass() {
    ...
    let u_ptr = addr_of!(u);
    let u1 = *u_ptr;
    let _padding = u1.a; // Read 2 bytes from `u`.
}
...
unsafe fn pointer_union_should_fail() {
    ...
    let u_ptr = addr_of!(u);
    let u1 = *u_ptr;
    let _padding = u1.b; // Read 4 bytes from `u`.
}
...
unsafe fn uber_union_should_pass() {
    ...
    let _non_padding = u.a; // Read 4 bytes from `u`.
}
...
unsafe fn uber_union_should_fail() {
    ...
    let _padding = u.a; // Read 4 bytes from `u`.
}
...
unsafe fn transmute_union_should_pass() {
    ...
    let _non_padding: u32 = std::mem::transmute(u); // Transmute `u` into a value of 4 bytes.
}
...
unsafe fn transmute_union_should_fail() {
    let u = U { a: 0 }; // `u` is initialized for 2 bytes.
    let _padding: u32 = std::mem::transmute(u); // Transmute `u` into a value of 4 bytes.
}
...
unsafe fn transmute_into_union_should_pass() {
    ...
    let _non_padding = u.b; // Read 4 bytes from `u`.
}
...
unsafe fn transmute_into_union_should_fail() {
    ...
    let _padding = u.b; // Read 4 bytes from `u`.
}
